...
int	msg(Client *client, Ircserv& serv, Command& command)
{
    ...
    for (std::vector<std::string>::iterator it = targets.begin(); it != targets.end(); it++)
    {
        ...
        if (serv.isChannel(*it))
        {
            ...
            Message mess(client->getFullname(), channel, message, false);
            client->addMessage(mess);
        }
        else
        {
            //Private message to a client
            Message mess(client->getFullname(), serv.getClient(*it), message, false);
            client->addMessage(mess);
        }
    }
    return (0);
...
int	notice(Client *client, Ircserv& serv, Command& command)
{
    ...
    for (std::vector<std::string>::iterator it = targets.begin(); it != targets.end(); it++)
    {
        ...
        if (serv.isChannel(*it))
        {
            ...
            Message mess(client->getFullname(), channel, message, true);
            client->addMessage(mess);
        }
        else
        {
            //Private message to a client
            Message mess(client->getFullname(), serv.getClient(*it), message, true);
            client->addMessage(mess);
        }
    }
    return (0);
...
int	wallops(Client *client, Ircserv& serv, Command& command)
{
    ...
    std::vector<Client *>   wallops = serv.getWallopsClients();
